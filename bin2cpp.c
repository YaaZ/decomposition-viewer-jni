 ...
 *
 * Modified a bit by Nikita Gubarkov into bin2cpp
 */
...
int
main(int argc, char *argv[]) // input file, output file, namespace, vector name
{
    char *buf;
    char *namespace;
    char *ident;
    ...
    fprintf(f_output, "#include <vector>\n\n");
    fprintf(f_output, "namespace %s {\n", namespace);
    fprintf(f_output, "extern const std::vector<unsigned char> %s = {", ident);
